...
func GetOAuthV2Response(client HTTPRequester, clientID, clientSecret, code, redirectURI string) (resp *OAuthV2Response, err error) {
...
func GetOAuthV2ResponseContext(ctx context.Context, client HTTPRequester, clientID, clientSecret, code, redirectURI string) (resp *OAuthV2Response, err error) {
	values := url.Values{
		"client_id":     {clientID},
		"client_secret": {clientSecret},
		"code":          {code},
	...
	if err = postForm(ctx, client, SLACK_API+"oauth.v2.access", values, response, false); err != nil {
		return nil, err
	}
